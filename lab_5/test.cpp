...
void win32Processing() {
    ...
    for (int i = 0; i < numOfThreads; ++i) {
        ...
        pThreads[i].hThread = CreateThread(NULL, 0, worker, (LPVOID)
        i, CREATE_SUSPENDED, NULL);
    }
...
for (
unsigned long int i = beginIndex;
i<endIndex;
i++) {
double xi = (i + 0.5) / N;
intermediatePi += 4 / (1 +
...
pThreads[dwTlsThreadIndex].threadPi +=
intermediatePi;

pThreads[dwTlsThreadIndex].
calculating = false;

SuspendThread(pThreads[dwTlsThreadIndex]
.hThread);
pThreads[dwTlsThreadIndex].
nextBlockIndex = InterlockedExchangeAdd(&nextBlock, 1);//nextBlock++;
