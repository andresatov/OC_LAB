...
Action::Action() {
    actionList = {{1,  std::bind(&Action::systemInfo, this)},
                  {2,  std::bind(&Action::virtualMemoryStatus, this)},
                  {3,  std::bind(&Action::virtualPageStatus, this)},
                  {4,  std::bind(&Action::separateReserveCommit, this)},
                  {5,  std::bind(&Action::simultaneousReserveCommit, this)},
                  {6,  std::bind(&Action::writeData, this)},
                  {7,  std::bind(&Action::protectVirtualPage, this)},
                  {8,  std::bind(&Action::freeVirtualPage, this)}
    };
}
...
void Action::virtualMemoryStatus() {
...
void Action::virtualPageStatus(DWORD address) {
...
void Action::separateReserveCommit() {
    PVOID pMemory = 0;
...
void Action::simultaneousReserveCommit() {
    PVOID pMemory = 0;
    PVOID pBaseAddress = 0;
    DWORD dwSize = 0;
    cout << "Базовый адрес (0 для автоматического задания адреса системой): 0x";
...
void Action::protectVirtualPage() {
    PVOID pMemory = 0;
    PVOID pBaseAddress = 0;
    DWORD dwSize = 0;
    cout << "Базовый адрес (0 для автоматического задания адреса системой): 0x";
...
void Action::freeVirtualPage(DWORD address) {
    PVOID pMemory = 0;
